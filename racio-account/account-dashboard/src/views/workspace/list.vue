<template>
        ...
        <el-row class="row-bg" :gutter="11">
            ...
            <el-col :span="24">
                <el-table :data="tableData" style="width: 100%">
                    ...
                    <el-table-column prop="role" label="帐号角色" width="120">
                        <template #default="scope">
                            <div>{{ scope.row.role == "owner" ? "空间所有者" : scope.row.role == "admin" ?
                                "空间管理员" : "尊享会员" }}</div>
                        </template>
                    </el-table-column>
    ...
    <el-dialog v-model="inviteDialog" title="邀请进入空间" width="500" align-center>
        <el-row>
            ...
            <el-select v-model="newRole">
                <template v-for="item in rolesList" :key="item.key">
                    <el-option :label="item.value" :value="item.key"
                        v-if="item.key !== currEditRoleInfo.account_role" />
                </template>
...
import { getAuthList, inviteUser, members, memberChangeRole, memberRemove } from "@/api/api"
