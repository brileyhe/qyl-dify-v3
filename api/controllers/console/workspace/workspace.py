...
class WorkspaceAccountMatchApi(Resource):
    ...
    def get(self):
        ...
        parser.add_argument(
            ...
            location="args",
            help="Role is optional. Choose one of the following: owner, admin, normal.",
        )
...
class WorkspaceAccountJoinApi(Resource):
    ...
    def post(self):
        ...
        parser.add_argument(
            ...
            default=TenantAccountJoinRole.OWNER.value,
            location="json",
            help="Role is required. choices: owner, admin, normal.",
        )
        parser.add_argument(
            "invited_by",
            type=str,
            ...
            default=None,
            location="json",
            help="Invited by account id is required.",
        )
        ...
        try:
            ...
            for tenant in tenants:
                ...
                tenant_account_join = (
                    ...
                    .filter(
                        TenantAccountJoin.tenant_id == tenant.id, TenantAccountJoin.account_id == UUID(str(account_id))
                    )
                    .first()
                )
            ...
            logging.exception(
                f"An error occurred during the WorkspaceAccountGetTenantsApi.get() process with: {str(e)}"
            )
            raise ValueError(str(e))
...
api.add_resource(
    WorkspaceAccountMatchApi, "/all-workspaces/match-account"
)  # GET for tenant by account and role via admin
api.add_resource(WorkspaceAccountJoinApi, "/all-workspaces/add-member")  # POST for joining tenant by account via admin
api.add_resource(
    WorkspaceAccountGetTenantsApi, "/all-workspaces/get-tenants"
)  # GET for getting all tenants by account id via admin
api.add_resource(
    WorkspaceAccountGetMembersApi, "/all-workspaces/get-members"
)  # GET for getting all accounts by tenant id via admin
