...
class ModelToolProviderController(ToolProviderController):
    ...
    def from_db(configuration: ProviderConfiguration = None) -> "ModelToolProviderController":
        ...
        if model_tool_configuration.label:
            ...
            label = I18nObject(en_US=configuration.provider.label.en_US, zh_Hans=configuration.provider.label.zh_Hans)
        ...
        return ModelToolProviderController(
            ...
            identity=ToolProviderIdentity(
                ...
                description=I18nObject(
                    zh_Hans=f"{label.zh_Hans} 模型能力提供商", en_US=f"{label.en_US} model capability provider"
                ),
                label=I18nObject(zh_Hans=label.zh_Hans, en_US=label.en_US),
    ...
    def _get_model_tools(self, tenant_id: str = None) -> list[ModelTool]:
        ...
        for model in configuration.get_provider_models():
            model_configuration = ModelToolConfigurationManager.get_model_configuration(
                self.configuration.provider.provider, model.model
            )
                ...
                tools.append(
                    ModelTool(
                        ...
                        description=ToolDescription(
                            ...
                            llm="Vision tool used to extract text and other visual information from images, "
                            "can be used for OCR, image captioning, etc.",
                        ),
